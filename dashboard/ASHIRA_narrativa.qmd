...
fig.update_layout(
    ...
    title=dict(
        text="Ocho años de datos. La distancia no cierra.<br><span style='color:#8b949e;'>Por cada paso que da el Sur, el Norte da dos.</span>",
        y=0.97, yanchor="top",
    ),
    legend=dict(
...
_top = _od2.head(80).copy()
_top["q"] = pd.qcut(_top["pob"], q=5, labels=False)

# ── Per-state balance ────────────────────────────────────────────────────────
_net2,_tot2 = {},{}
for _s in _ctr_d:
    _i = _od2[_od2["gd"]==_s]["pob"].sum(); _o = _od2[_od2["go"]==_s]["pob"].sum()
    _net2[_s]=int(_i-_o); _tot2[_s]=int(_i+_o)
_mt2 = max(_tot2.values())

# ── Serialize to JSON ────────────────────────────────────────────────────────
import json as _json
_arcs_js = _json.dumps([
    {"srcLat":_ctr_d[r.go][1],"srcLng":_ctr_d[r.go][0],
     "dstLat":_ctr_d[r.gd][1],"dstLng":_ctr_d[r.gd][0],
     "pob":int(r.pob),"q":int(r.q),"src":r.go,"dst":r.gd}
    for _,r in _top.iterrows()], separators=(",",":"))
_nodes_js = _json.dumps([
    {"state":s,"lat":_ctr_d[s][1],"lng":_ctr_d[s][0],"net":_net2[s],"tot":_tot2[s]}
    for s in _ctr_d], separators=(",",":"))
_geo_js = _json.dumps(mx_geo, separators=(",",":"))

_html_globe = f"""
<div class="ashira-globe-card" style="font-family:Inter,sans-serif;border-radius:14px;overflow:hidden;margin:8px 0 4px;position:relative;">
  <div class="ashira-globe-header" style="padding:22px 30px 10px;">
    <h3 class="ashira-globe-title" style="margin:0 0 5px;font-family:Fraunces,serif;font-size:1.25em;font-weight:600;">
      &#191;Ad&#243;nde va la gente que se va?
    </h3>
    <p class="ashira-globe-subtitle" style="margin:0;font-size:0.85em;">
      Flujos interestatales reales 2015&#8211;2020 &nbsp;&#183;&nbsp; Top 80 de 992 pares &nbsp;&#183;&nbsp;
      Arrastra para rotar &nbsp;&#183;&nbsp; Scroll para zoom &nbsp;&#183;&nbsp; Hover sobre arcos y estados
    </p>
  </div>
  <div id="globe-flujo-ashira" style="width:100%;height:520px;background:#040c18;display:flex;align-items:center;justify-content:center;">
    <span id="globe-loading-msg" style="color:#484f58;font-size:13px;font-family:Inter,sans-serif;">Cargando globo WebGL...</span>
  </div>
  <div class="ashira-globe-legend" style="position:absolute;bottom:0;left:0;right:0;z-index:20;background:rgba(4,12,24,0.92);padding:10px 22px 12px;display:flex;gap:16px;align-items:center;flex-wrap:wrap;row-gap:8px;border-top:1px solid rgba(26,60,92,0.6);pointer-events:none;">
    <span class="ashira-globe-legend-head" style="color:#3d5268;font-size:0.68em;font-family:Inter,sans-serif;text-transform:uppercase;letter-spacing:0.08em;font-weight:600;">Puntos</span>
    <span style="display:flex;align-items:center;gap:7px;">
      <span style="width:11px;height:11px;border-radius:50%;background:#26c6da;display:inline-block;box-shadow:0 0 5px #26c6da99;flex-shrink:0;"></span>
      <span class="ashira-globe-legend-text" style="color:#8ba4be;font-size:0.79em;font-family:Inter,sans-serif;">Receptor neto de migraci&#243;n</span>
    </span>
    <span style="display:flex;align-items:center;gap:7px;">
      <span style="width:11px;height:11px;border-radius:50%;background:#ff5252;display:inline-block;box-shadow:0 0 5px #ff525299;flex-shrink:0;"></span>
      <span class="ashira-globe-legend-text" style="color:#8ba4be;font-size:0.79em;font-family:Inter,sans-serif;">Emisor neto de migraci&#243;n</span>
    </span>
    <span style="width:1px;height:16px;background:rgba(26,60,92,0.8);display:inline-block;margin:0 2px;flex-shrink:0;"></span>
    <span class="ashira-globe-legend-head" style="color:#3d5268;font-size:0.68em;font-family:Inter,sans-serif;text-transform:uppercase;letter-spacing:0.08em;font-weight:600;">Arcos</span>
    <span style="display:flex;align-items:center;gap:7px;">
      <span style="width:22px;height:2px;background:linear-gradient(90deg,rgba(30,100,255,0.5),rgba(80,160,255,0.95));display:inline-block;border-radius:1px;flex-shrink:0;"></span>
      <span class="ashira-globe-legend-text" style="color:#8ba4be;font-size:0.79em;font-family:Inter,sans-serif;">Menor volumen</span>
    </span>
    <span style="display:flex;align-items:center;gap:7px;">
      <span style="width:22px;height:2px;background:linear-gradient(90deg,rgba(255,60,0,0.6),rgba(255,100,30,0.98));display:inline-block;border-radius:1px;flex-shrink:0;"></span>
      <span class="ashira-globe-legend-text" style="color:#8ba4be;font-size:0.79em;font-family:Inter,sans-serif;">Mayor volumen</span>
    </span>
    <span class="ashira-globe-legend-src" style="margin-left:auto;color:#273848;font-size:0.68em;font-family:Inter,sans-serif;">INEGI &#183; Censo 2020</span>
  </div>
</div>

<!-- Paso 1: datos embebidos (sin dependencias externas) -->
<script>
var _gA={_arcs_js};
var _gN={_nodes_js};
var _gG={_geo_js};
...
<!-- Paso 2: inicialización (Globe.gl ya cargó desde el header del documento) -->
<script>
(function(){{
  var _msg=document.getElementById('globe-loading-msg');
  function _initGlobeAshira(){{
  var el=document.getElementById('globe-flujo-ashira');
  if(!el)return;
  if(_msg){{_msg.style.display='none';}}
  el.style.display='block';el.style.alignItems='';el.style.justifyContent='';
  var _lm=document.documentElement.classList.contains('light-mode');
  var ARC_COLORS=_lm?[
    ['rgba(0,70,200,0.50)','rgba(40,120,255,0.90)'],
    ['rgba(0,140,200,0.55)','rgba(0,190,235,0.92)'],
    ['rgba(0,170,155,0.58)','rgba(0,210,185,0.95)'],
    ['rgba(200,110,0,0.62)','rgba(245,165,10,0.95)'],
    ['rgba(200,25,0,0.66)','rgba(245,65,10,0.98)']
  ]:[
    ['rgba(30,100,255,0.12)','rgba(80,160,255,0.50)'],
    ['rgba(0,180,230,0.15)','rgba(0,220,255,0.58)'],
    ['rgba(0,230,200,0.18)','rgba(0,255,220,0.66)'],
    ['rgba(255,165,0,0.20)','rgba(255,210,40,0.72)'],
    ['rgba(255,60,0,0.24)','rgba(255,100,30,0.82)']
  ];
  var maxTot=Math.max.apply(null,_gN.map(function(n){{return n.tot;}}));
  try{{
    var g=Globe()
      .globeImageUrl('')
      .backgroundColor(_lm?'#dde6f0':'#040c18')
      .showAtmosphere(true)
      .atmosphereColor(_lm?'#6aaad4':'#0a2040')
      .atmosphereAltitude(0.18)
      .polygonsData(_gG.features)
      .polygonCapColor(function(){{return _lm?'#bfd0e0':'#0c1825';}})
      .polygonSideColor(function(){{return _lm?'#aec2d4':'#081218';}})
      .polygonStrokeColor(function(){{return _lm?'#5a90b8':'#1a3a5c';}})
      ...
      .arcColor(function(d){{return ARC_COLORS[d.q];}})
      .arcAltitude(function(d){{return 0.03+d.q*0.06;}})
      .arcStroke(function(d){{return 0.08+d.q*0.06;}})
      .arcDashLength(0.14)
      .arcDashGap(0.86)
      .arcDashInitialGap(function(d){{return Math.random();}})
      .arcDashAnimateTime(function(d){{return 2200+(4-d.q)*1600;}})
      .arcLabel(function(d){{
        return '<div style="background:rgba(4,12,24,0.96);padding:9px 13px;border-radius:8px;font-family:Inter,sans-serif;border:1px solid rgba(255,255,255,0.12);max-width:220px">'
          +'<b style="font-size:13px;color:#e6edf3">'+d.src+' → '+d.dst+'</b><br>'
          +'<span style="color:#8b949e;font-size:11px">Personas: <b style="color:#c9d1d9">'+d.pob.toLocaleString('es-MX')+'</b></span>'
          +'</div>';
      }})
      .pointsData(_gN)
      .pointLat(function(d){{return d.lat;}})
      .pointLng(function(d){{return d.lng;}})
      .pointColor(function(d){{return d.net>0?'#26c6da':'#ff5252';}})
      .pointAltitude(0.03)
      .pointRadius(function(d){{return 0.10+0.28*Math.pow(d.tot/maxTot,0.38);}})
      .pointLabel(function(d){{
        var sign=d.net>0?'+':'';
        var col=d.net>0?'#26c6da':'#ff5252';
        return '<div style="background:rgba(4,12,24,0.96);padding:9px 13px;border-radius:8px;font-family:Inter,sans-serif;border:1px solid rgba(255,255,255,0.12)">'
          +'<b style="font-size:13px;color:#e6edf3">'+d.state+'</b><br>'
          +'<span style="color:#8b949e;font-size:11px">Flujo total: <b style="color:#c9d1d9">'+d.tot.toLocaleString('es-MX')+'</b></span><br>'
          +'<span style="color:#8b949e;font-size:11px">Balance: <b style="color:'+col+'">'+sign+d.net.toLocaleString('es-MX')+'</b></span>'
          +'</div>';
      }})
      (el);
    g.pointOfView({{lat:16,lng:-100,altitude:0.82}},0);
    g.controls().enableZoom=true;
    g.controls().autoRotate=false;
    g.controls().enablePan=false;
    g.controls().minPolarAngle=Math.PI*0.18;
    g.controls().maxPolarAngle=Math.PI*0.82;
    var _oT=Date.now(),_oA=true,_oPer=26000;
    var _cLng=-100,_lRng=1,_rRng=20,_lF=_lRng/(_lRng+_rRng);
    function _osc(){{
      if(_oA){{
        var _tN=((Date.now()-_oT)%_oPer)/_oPer,_ph,_lng;
        if(_tN<_lF){{_ph=_tN/_lF*Math.PI;_lng=_cLng-_lRng*Math.sin(_ph);}}
        else{{_ph=(_tN-_lF)/(1-_lF)*Math.PI;_lng=_cLng+_rRng*Math.sin(_ph);}}
        g.pointOfView({{lat:16,lng:_lng,altitude:0.82}});
      }}
      requestAnimationFrame(_osc);
    }}
    g.controls().addEventListener('start',function(){{_oA=false;}});
    g.controls().addEventListener('end',function(){{setTimeout(function(){{_oT=Date.now();_oA=true;}},2500);}});
    _osc();
  }}catch(e){{
    el.innerHTML='<p style="color:#ff5252;padding:20px;font-family:Inter">Error al inicializar el globo: '+e.message+'</p>';
  }}
  }}
  if(window.Globe){{
    _initGlobeAshira();
    ...
    _s.src='globe.gl.min.js';
    _s.onload=_initGlobeAshira;
    _s.onerror=function(){{
      if(_msg){{_msg.style.color='#ff5252';_msg.textContent='Error: No se pudo cargar globe.gl.min.js';}}
    }};
...
<span class="que-sigue-card-icon"><lord-icon src="https://cdn.lordicon.com/bhfjfgqz.json" trigger="hover" colors="primary:#e94560,secondary:#38bdf8" style="width:56px;height:56px"></lord-icon></span>
...
<div class="que-sigue-card">
<span class="que-sigue-card-icon"><lord-icon src="https://cdn.lordicon.com/wjyqkiew.json" trigger="hover" colors="primary:#e94560,secondary:#38bdf8" style="width:56px;height:56px"></lord-icon></span>
...
<div class="que-sigue-card">
<span class="que-sigue-card-icon"><lord-icon src="https://cdn.lordicon.com/bduzytli.json" trigger="hover" colors="primary:#e94560,secondary:#38bdf8" style="width:56px;height:56px"></lord-icon></span>
...
<div class="que-sigue-card">
<span class="que-sigue-card-icon"><lord-icon src="https://cdn.lordicon.com/jdgfsfzr.json" trigger="hover" colors="primary:#e94560,secondary:#38bdf8" style="width:56px;height:56px"></lord-icon></span>
...
<a href="#inicio" class="cta-share-btn link"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12l7-7 7 7"/><path d="M12 5v14"/></svg>Volver al inicio</a>
</div>
```
...
<footer class="ashira-footer">
  <div class="ashira-footer-inner">
    <div class="ashira-footer-brand">
      <span class="ashira-footer-logo">ASHIRA</span>
      <span class="ashira-footer-tag">HackODS UNAM 2026 · ODS 1 y ODS 10</span>
    </div>
    <div class="ashira-footer-sources">
      <span class="ashira-footer-label">Fuentes</span>
      <span>ENIGH 2016–2024 · PM CONEVAL 2024 · Censo 2020 · ICE IMCO 2020 · Migración OD Censo 2020</span>
      <span>INEGI · CONEVAL · IMCO · Licencia CC BY-SA 4.0</span>
    </div>
  </div>
</footer>
```